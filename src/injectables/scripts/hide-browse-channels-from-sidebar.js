(function() {
  function getElement() {
    return Array.from(
      Array.from(document.getElementsByTagName('ytd-guide-section-renderer')).filter(x => x.getElementsByTagName('h3')[0].innerText === 'Subscriptions')[0]?.getElementsByTagName('ytd-guide-entry-renderer') ?? []
    ).filter(x => x.innerText === 'Browse channels')[0];
  }
